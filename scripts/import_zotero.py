...
"""Import a Zotero library into prax.
    ...
    python scripts/import_zotero.py ~/Zotero --dry-run            # inventory only
    python scripts/import_zotero.py ~/Zotero --dry-run --hash     # + sha256 dedupe
    python scripts/import_zotero.py ~/Zotero --commit --limit 500 # trial run
    python scripts/import_zotero.py ~/Zotero --commit             # the whole library
