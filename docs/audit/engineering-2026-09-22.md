...
audit, done in one commit (`d774141`). The glyph check remembers each
text it read (`repair._glyphs_seen`, keyed by `text_hash`). The unread
figures are a partial index (migration 17), so their check is an index
scan. The standing questions' listing is kept while the store's change
stamp stands. A figure is served only under a document whose text
references it. llama-server bound beyond loopback is started with the
model's `api_key_env` as its `--api-key`, or the supervisor's log says
it has none.
