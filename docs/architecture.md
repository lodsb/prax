...
| resolve step | an entity type whose likely pairs are a week old or were never computed | `entity_candidates` for that type, replaced whole | the door never embeds a name; the worker does, a block at a time |
