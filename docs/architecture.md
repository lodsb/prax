...
| `prax.references` | a reference list read by rules: the entries split (numbered, listed, author-year, Elsevier's one-paragraph lists), each into surnames, year, title and a printed id; a score against a library document's title, creators and year; the decision with a threshold and a margin. The `references` pass of `prax maintain` writes the `cites` edges | no |
