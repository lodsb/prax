...
| `GET /work/{step}?limit&scope`, `POST /work/{step}`, `POST /work/session`, `POST /work/session/{id}`, `POST /vectors/merge` | the work protocol: a leased batch for parse, titles, extract or embed (`scope` is captures or all); the results, applied by the door; a worker's session job with heartbeats; the delta indexes folded into the main files |
