...
| `GET /models/servers` | the `openai` model servers of `prax.yaml`, one entry per server: reachable, alias, model file, slots, vision, and the load from `/metrics` when the server exposes it |
...
| `#jobs` | the batch passes running now (name, progress bar, done/total, note, heartbeat, host) and the recent ones; the navigation shows a badge with the running count; above them the model servers `prax.yaml` names (`GET /models/servers`): reachable or not, model file, slots, whether it sees images, and — when started with `--metrics` — requests running and waiting, KV cache use, tokens per second |
