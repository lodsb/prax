...
| `POST /ingest/file` (multipart `file`, `title`, `domains`, `tags`, `session`), `POST /ingest/html {url, html, title, domains, tags, session}`, `POST /ingest/url {url, title, domains, tags, session}`, `GET /inbox?limit` | captures: an uploaded file, a page as the browser rendered it, a URL the door fetches; each returns the document id, whether it was new, whether it is searchable already, its domains and the previous capture of the same URL; `/inbox` lists recent captures with state, the drop folder path and the domains to choose from |
...
| `#inbox` | captures: a drop zone and file picker (several files at once), domain checkboxes and tags applied to the upload, a title for a single file; a URL form the door fetches; the drop folder's path; the recent captures with source, domains, time and state (pending, indexed, extracted); each result links to its document |
