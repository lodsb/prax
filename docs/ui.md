...
| `GET /work/{step}?limit&scope`, `POST /work/{step}`, `POST /work/session`, `POST /work/session/{id}`, `POST /vectors/merge` | the work protocol: a leased batch for parse, titles, summaries, vocabulary, sections, extract or embed (`scope` is captures or all); the results, applied by the door; a worker's session job with heartbeats; the delta indexes folded into the main files |
...
| `POST /graph/label {entity_id, label, lang, kind, producer, run}` | a name an entity is also known by; a preferred one displaces the one already preferred in that language and the shown name follows |
| `POST /graph/retire {run, producer}` | every live edge a run or a producer wrote ended, the rows kept with the moment they stopped being true (invariant 8's own mechanism; `prax resolve --retire`) |
