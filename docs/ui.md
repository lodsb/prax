...
| `POST /doc/{id}/reading {extractor, mode}`, `DELETE /doc/{id}/reading`, `GET /readings` | ask for a named extractor on one document (`vision-pages` with `scans` or `all`, `figures` with `captioned` or `all`, `vision`, `pymupdf4llm-ocr`, `docling`; validated against the type, the mode against the extractor); withdraw or dismiss it; the waiting requests and the recently finished ones with their outcome, plus what the vision step resolves to. The work protocol hands requests out before the pending captures, forced, with the mode; a worker refuses one whose model would cost money and says so in the outcome |
