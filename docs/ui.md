...
| `#pages` (standing questions listed first with what is new for each — "1 new document · ask again", or "settled · ask again anyway" — `GET /questions`, `POST /questions/run`; briefings in a group of their own; a question page's action row has "ask again", its meta line says when it was asked and how often it moved) | the wiki: create topic and project pages; lists by kind with revision and author; a row opens the document view with the editor |
