...
| `#jobs` | the batch passes running now (name, progress bar, done/total, note, heartbeat, host) and the recent ones; the navigation shows a badge with the running count; the readings asked for (waiting, and what came back with its outcome) and which model the vision step resolves to; at the foot, Health: what every ailment of `prax heal` finds right now (`GET /heal`) and a button that repairs the repairable ones (`POST /heal`, a job; nothing is deleted); above them the model servers `prax.yaml` names (`GET /models/servers`): reachable or not, model file, slots, whether it sees images, and — when started with `--metrics` — requests running and waiting, tokens per second, tokens read and the share the prompt cache served |
